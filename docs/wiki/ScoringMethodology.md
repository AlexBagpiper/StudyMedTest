...
1.  **Порог «идеальной точности» (Grace Zone)**: Если средняя точность (IoU) всех найденных объектов выше установленного порога (напр. 95%), она автоматически приравнивается к 1.0 (100%).
2.  **Бонус за безошибочность (Loyalty Boost)**: Если студент нашел абсолютно все объекты эталона (Полнота (Recall)=100%) и не добавил ни одной лишней аннотации (Точность Precision=100%), к его показателю точности добавляется настраиваемый бонус (напр. +5%), но не более 100%. Величина бонуса задается в настройках системы.
3.  **Округление итога (Top-off Rule)**: Если итоговый взвешенный балл за графический вопрос превышает критический порог (напр. 99.0%), система округляет его до 100.0%.
...
*   *С Grace Zone (95%):* IoU не дотягивает до 100%.
*   *С Loyalty Boost (+5%):* IoU становится 94 + 5 = 99%. Итоговый балл вырастает до 99.5%.
*   *С Top-off Rule (99.0%):* Итоговый балл 99.5% округляется до **100.0%**.
