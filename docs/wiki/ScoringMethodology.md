...
*   **Частичный зачет**: Режим для сложных и крупных объектов (например, контуры тканей), где полное выделение слишком трудоемко. В этом режиме используются две простые метрики:
...
*   **Условия зачета**: Объект считается найденным, если студент попал точно (например **Inclusion >= 80%**) и выделил хотя бы минимально значимую часть (например **Coverage >= 5%**). При выполнении этих условий объект засчитывается в метрику Полноты (Recall) на 100%.
*   **Расчет точности при частичном зачете**:
    В этом режиме Геометрическая точность оценивается по показателю **Inclusion** (насколько аккуратно студент не выходил за границы эталона). Это позволяет студенту получить 100% за точность, даже если он выделил лишь малую часть объекта, подтвердив свое понимание вопроса.
...
### 2.1. Гибкая оценка по отдельным меткам

Система позволяет настраивать требования к каждой категории объектов (метке) индивидуально. Это делает одну и ту же разметку пригодной для разных типов вопросов.

#### Режимы оценки меток:
1.  **Все контуры**: Студент должен найти абсолютно все объекты данной метки, указанные в эталоне. Полнота (Recall) для этой группы рассчитывается как `найдено / всего`.
2.  **Не менее**: Достаточно найти заданное количество объектов. Например, если в эталоне 10 метастазов, а в настройках указано "Не менее 1", то при нахождении любого метастаза студент получит Recall = 100% за эту группу. Максимальное значение N ограничено количеством контуров этой метки в эталоне.

#### Индивидуальные параметры для меток:
*   **Вес метки**: Коэффициент значимости (от 1 до 5). Определяет вклад Recall данной метки в общую оценку полноты вопроса. Чем выше вес, тем важнее найти объекты этой категории.
*   **Частичный зачет**: Настройка может быть включена индивидуально для каждой метки. Если включена, для этой категории будет использоваться метрика **Inclusion** вместо IoU (удобно для крупных органов или тканей, где важен факт попадания в зону, а не идеальный контур).

#### Глобальная точность:
Показатель точности (Precision) рассчитывается по всему вопросу. Если студент нарисовал контуры меток, которые **не включены** в активные критерии оценки данного вопроса, они будут считаться лишними и снизят балл за точность. Это предотвращает "закрашивание" всего снимка для получения баллов.

#### Итоговая агрегация:
Итоговые показатели Полноты (Recall) и Геометрической точности (IoU) для вопроса рассчитываются как средневзвешенные значения всех активных меток:

$$Recall_{total} = \frac{\sum (Recall_{label} \times Weight_{label})}{\sum Weight_{label}}$$

$$IoU_{total} = \frac{\sum (Accuracy_{label} \times Weight_{label})}{\sum Weight_{label}}$$

Такой подход гарантирует, что ошибки в точности или пропуски «важных» объектов (с высоким весом) будут влиять на итоговый балл значительно сильнее, чем погрешности в менее значимых элементах.

### Механизмы лояльности в графических заданиях

Для снижения психологического давления и компенсации физиологических погрешностей (тремор рук, разрешение экрана) в системе реализован «Режим лояльности». Он позволяет студентам получать 100% за задание, если они выполнили его клинически верно, но допустили незначительные геометрические отклонения.

1.  **Порог «идеальной точности»**: Если средняя точность (IoU) всех найденных объектов выше установленного порога (напр. 95%), она автоматически приравнивается к 1.0 (100%).
2.  **Бонус за безошибочность**: Если студент нашел абсолютно все объекты эталона (Полнота (Recall)=100%) и не добавил ни одной лишней аннотации (Точность Precision=100%), к его показателю точности добавляется настраиваемый бонус (напр. +5%), но не более 100%. Величина бонуса задается в настройках системы.
3.  **Округление итога**: Если итоговый взвешенный балл за графический вопрос превышает критический порог (напр. 99.0%), система округляет его до 100.0%.

Эти механизмы поощряют правильное клиническое мышление и понимание анатомии, не наказывая за пиксельную неточность.

**Пример работы лояльности:**
Студент нашел опухоль (Recall=100%), не сделал лишних пометок (Precision=100%), но из-за дрожания руки точность контура составила IoU=94%.
*   *Без лояльности:* Итоговый балл за вопрос ~95.5% (при стандартных весах).
*   *С "Порог «идеальной точности»" (95%):* IoU не дотягивает до 100%.
*   *С "Бонус за безошибочность" (+5%):* IoU становится 94 + 5 = 99%. Итоговый балл вырастает до 99.5%.
*   *С "Округление итога" (99.0%):* Итоговый балл 99.5% округляется до **100.0%**.
